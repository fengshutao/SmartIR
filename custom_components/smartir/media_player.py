...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry, async_add_entities):
    """Set up the IR Media Player platform from a config entry."""
    config = dict(entry.options)
    config[CONF_NAME] = entry.title
    
    # Parse source_names string to dict if provided
    source_names = config.get("source_names")
    if source_names and isinstance(source_names, str):
        try:
            import ast
            config[CONF_SOURCE_NAMES] = ast.literal_eval(source_names)
        except:
            _LOGGER.warning("Could not parse source_names, using as-is")
            config[CONF_SOURCE_NAMES] = {}
    
    await _async_setup_entity(hass, config, async_add_entities)

async def _async_setup_entity(hass, config, async_add_entities):
    """Set up a single SmartIR media player entity."""
...
class SmartIRMediaPlayer(MediaPlayerEntity, RestoreEntity):
    def __init__(self, hass, config, device_data):
        ...
        if 'sources' in self._commands and self._commands['sources'] is not None:
            ...
            # Parse source_names from config
            source_names = config.get(CONF_SOURCE_NAMES, {})
            if isinstance(source_names, dict):
                for source, new_name in source_names.items():
