...
async def async_setup_entry(hass, entry: ConfigEntry):
    ...
    device_type = entry.options.get("device_type", "climate")
    
    # 修复：使用 async_forward_entry_setups (复数形式)
...
async def _async_update_listener(hass, entry: ConfigEntry):
...
async def async_unload_entry(hass, entry: ConfigEntry):
    ...
    # 修复：使用 async_unload_entry_setups (复数形式)
    if device_type == "climate":
        return await hass.config_entries.async_unload_entry_setups(entry, ["climate"])
    elif device_type == "fan":
        return await hass.config_entries.async_unload_entry_setups(entry, ["fan"])
    elif device_type == "light":
        return await hass.config_entries.async_unload_entry_setups(entry, ["light"])
    elif device_type == "media_player":
        return await hass.config_entries.async_unload_entry_setups(entry, ["media_player"])
